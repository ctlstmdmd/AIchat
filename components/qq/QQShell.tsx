...
const navItems: Array<{ id: View; label: string; icon: IconName; href: string }> = [
  { id: "messages", label: "消息", icon: "message", href: "/messages" },
  { id: "contacts", label: "联系人", icon: "contacts", href: "/contacts" },
  { id: "zone", label: "空间", icon: "zone", href: "/zone" },
...
function Rail({ current, mode }: { current: View; mode: AppSettings["mode"] }) {
  return <aside className="rail" aria-label="主导航">
    ...
    <nav className="rail-nav">{navItems.map((item) => <Link className="rail-button" key={item.id} href={item.href} aria-current={current === item.id ? "page" : undefined} aria-label={item.label} title={item.label}><Icon name={item.icon} />{item.id === "messages" && <span className="nav-badge">2</span>}</Link>)}</nav>
    <div className="rail-bottom"><span className={`mode-indicator ${mode}`} title={mode === "demo" ? "Demo Mode" : "Live AI Mode"}><i />{mode === "demo" ? "DEMO" : "LIVE"}</span><Link className="rail-button" href="/settings" aria-current={current === "settings" ? "page" : undefined} aria-label="设置" title="设置"><Icon name="settings" /></Link></div>
  </aside>;
}
...
function ChannelsView() { return <div className="content-view simple-view"><header className="content-topbar"><div><p className="eyebrow">Channels</p><h2>频道</h2></div></header><div className="simple-content"><div className="quiet-state"><span className="icon-surface"><Icon name="channel" /></span><h3>频道会在后续阶段开放</h3><p>这一阶段专注于单角色聊天和作品集体验。</p></div></div></div>; }
function MobileNav({ current }: { current: View }) { return <nav className="mobile-nav" aria-label="移动端导航">{navItems.filter((item) => ["messages", "zone", "memories", "profile"].includes(item.id)).map((item) => <Link key={item.id} href={item.href} className={current === item.id ? "active" : ""}><Icon name={item.icon} size={21} /><span>{item.label}</span></Link>)}<Link href="/settings" className={current === "settings" ? "active" : ""}><Icon name="settings" size={21} /><span>设置</span></Link></nav>; }
...
function Avatar({ character, mine = false, size = "md", group = false, initials, tone }: { character?: Character; mine?: boolean; size?: "sm" | "md" | "lg" | "xl" | "hero"; group?: boolean; initials?: string; tone?: "coral" | "violet" }) {
  ...
  return <span className={`avatar avatar-${size} ${mine ? "mine-avatar" : ""} ${group ? "group-avatar" : ""} ${tone ?? ""}`} aria-label={label}>{character?.avatar ? <Image src={character.avatar} alt="" fill sizes={size === "hero" ? "116px" : size === "xl" ? "72px" : "48px"} /> : <b>{letters}</b>}{(character?.status === "online" || mine) && <i className="status-dot" />}</span>;
}
